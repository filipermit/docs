---
sidebar_position: 1
title: Migrating the API - v1 to v2
---

:::info
You can find the new V2 API **[here](https://api.permit.io/v2/redo)**. 🚀

If you need further help or have questions, please reach out to our team on **[Slack](https://permit-io.slack.com/join/shared_invite/zt-nz6yjgnp-RlP9rtOPwO0n0aH_vLbmBQ#/shared-invite/email)**. 💁
:::
...
### TL;DR - What changed overview

:::info Strict Hierarchy of Objects
Organizations contain Projects which contain Environments, which with few exceptions contain all objects you're familiar with.

All objects are divided into **general**, **schema** and **facts** objects, and can be found under their respective roots.

**General objects**: Organizations, Projects, Environments.<br/>
**Schema objects**: Roles, Resources, Actions, and Condition Sets.<br/>
**Facts objects**: Tenants, Users, Role Assignments, and Condition Set Rules.<br/>
:::

:::info Slugs and Custom ID's have all been replaced with Keys
Keys are immutable strings unique under their respective namespaces, and can be used wherever Permit-'s auto-generated UUIDs are used. They enable better looking URLs and simpler integration with your system's native naming rules.
:::

#### Organization-scoped schema objects are no longer supported
...
#### V1 API remains active, but it is deprecated and no longer maintained

The v1 API can be used and will show the same schema and data as v2, as will v1 PDPs, but they're deprecated and we recommend you move to v2 as soon as it is convenient. You can use both APIs interchangeably, but except quirks as they have different assumptions about the structure of data.

#### SDK functionality has been fully redesigned

The SDKs have become easier and more convenient to use, but are not compatible with the v1 SDKs. If your code writes using the SDKs, you'll have to modify it to use the new calls. Note that if the only write you're doing is sync_user, the change is trivial to implement. See [Migrating the PDP and SDK](#migrating-the-pdp-and-sdk) for how to do that.
